...
namespace engone {
	...
	/*
		...
		IMPORTANT: Virtual functions are not used because they don't work so well with dlls. Especially with other libraries like GLEW and GLFW.
		   I am probably not doing things correctly and there are ways to deal with all this but the easiest would be to just not use virtual
		   functions. We don't even need to use virtual functions here. I think I was just having a moment of the object oriented mindset.
	*/
	...
	public:
		...
		// Virtual methods are not used because they are tricky with dlls (vtable).
	...
	public:
		...
		// size in bytes
		void getData(uint32_t size, void* data, uint32_t offset = 0);
